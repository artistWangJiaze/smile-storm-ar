// LOCAL-ONLY motion study. Collision code is unchanged from the production integrator.
// The sourcePosition function changes only the pre-contact bloom choreography.
...
vec2 sourcePosition(vec2 uv) {
  float age=uDispersion;
  float grow=smoothstep(0.0,0.46,age);
  float expand=smoothstep(0.46,2.75,age);
  float dissolve=smoothstep(0.65,3.0,age);
  float x=float(gl_VertexID%300),y=float(gl_VertexID/300);
  float a=hash(vec3(x,y,11)),b=hash(vec3(x,y,23)),c=hash(vec3(x,y,41));
  vec2 p=uv-0.5+(vec2(b,c)-0.5)/300.0*0.85;
  vec2 base=p*640.0*uScale*mix(0.025,1.0,grow);
  float r=length(base);
  vec2 dir=r>0.001?base/r:vec2(1,0), tangent=vec2(-dir.y,dir.x);
  float noiseA=noise(vec3(p*7.0,uTime*0.09+uSeed));
  float noiseB=noise(vec3(p*12.0,uTime*0.13+uSeed));
  // Preserve the whole ornament through initial bloom. Its own samples then
  // peel outward, with varying radial speeds instead of stretching only its rim.
  float halo=pow(a,7.0);
  float outer=smoothstep(0.045,0.25,length(p));
  float release=smoothstep(0.64,1.65,age);
  vec2 target=base*(1.0+expand*mix(0.25,1.05,outer));
  target+=dir*(halo*(12.0+150.0*release)*uScale*outer+(noiseA-0.5)*r*0.22*dissolve);
  target+=tangent*(noiseB-0.5)*r*0.16*dissolve;
  target+=(vec2(b,c)-0.5)*110.0*uScale*pow(dissolve,1.6);
...
export class TDParticleSimulation {
  ...
  constructor(private readonly gl: WebGL2RenderingContext) {
    ...
    for (const name of ['uImage','uOrigin','uTime','uDt','uScale','uDispersion','uSeed','uReset','uHeadCount','uSiteCount','uHead[0]','uHeadCenter','uHeadVelocity','uHeadBounds','uSites[0]']) {
